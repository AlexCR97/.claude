...
description: Deprecated alias for ticket-digest with source=ado. Use /ticket-digest ado:{id} instead.
argument-hint: "<id>"
disable-model-invocation: true
---

This skill is a **deprecated alias**. It contains no logic of its own: it prefixes the work item ID with the `ado:` source
and hands off to `ticket-digest`.
...
2. Tell the user, in **exactly one line**:
   ...
   > `/az-workitem-digest {id}` is now `/ticket-digest ado:{id}`. Running it for you.
