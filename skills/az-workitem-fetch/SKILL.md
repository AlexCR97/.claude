...
description: Deprecated alias for ticket-fetch with source=ado. Use /ticket-fetch ado:{id} instead.
argument-hint: "<id>"
disable-model-invocation: true
---

This skill is a **deprecated alias**. It contains no logic of its own: it prefixes the work item ID with the `ado:` source
and hands off to `ticket-fetch`.
...
2. Tell the user, in **exactly one line**:
   ...
   > `/az-workitem-fetch {id}` is now `/ticket-fetch ado:{id}`. Running it for you.
